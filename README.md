...
The `TilingModule` class has been tested and is confirmed to work with the following PyTorch Data types / dtypes: `torch.float32` / `torch.float`, `torch.float64` / `torch.double`, `torch.float16` / `torch.half`, & `torch.bfloat16`.
